...
export function SocialPlatformPill({
  className,
  label,
  network,
  onPress
}: SocialPlatformPillProps) {
  return (
    <PressableFeedback
      animation={{ scale: { value: 0.995 } }}
      className={cn(
        'min-h-[66px] min-w-[164px] shrink-0 flex-1 flex-row items-center gap-3 rounded-lg border border-border bg-surface px-5',
        className
      )}
      onPress={onPress}>
